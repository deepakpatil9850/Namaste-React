...
const MenuList = () =>{
    ...
    const menuCategories = cards?.cards?.filter(card => card?.card?.card["@type"] === "type.googleapis.com/swiggy.presentation.food.v2.ItemCategory");
            ...
            <div className="w-6/12 mx-auto border-2 border-black">
               ...
               {            // Accordion
                    menuCategories.map((item , index) => 
                            <MenuCategory  key={item?.card?.card?.title }   data={item?.card?.card}   isActive = { activeIndex === index} onShow = {()=> setActiveIndex(index)}  />
                    ) 
               }
            </div>
        </div>
